...
class FQI:
    ...
    def evaluate(self):
        ...
        for episodes in range(self.config.evaluation_episodes):
            ...
            print(episodes)
            while not done:
                q_values = [self.model(torch.cat((torch.tensor(one_hot(state), dtype=torch.float32, device=self.device).unsqueeze(0), torch.tensor(one_hot(a), dtype=torch.float32, device=self.device).unsqueeze(0)), dim=0)).detach().cpu().numpy() for a in range(env.action_space.n)]
                print(q_values)
